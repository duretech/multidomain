  ...
  methods: {
      ...
      if (formValues && formValues[2]) {
        ...
        import(/* webpackChunkName: "pptxgenjs"*/ "pptxgenjs").then(
          async (pptxgen) => {
            ...
            if (this.$store.getters.getActiveTab.includes("sd-summary")) {
              ...
              if (this.showTrend) {
                this.exportArrAll.map((item) => {
                  if (
                    item.chartCategory === "trend" &&
                    item.path == "sd-summary" &&
                    !item.viewMore &&
              ...
              if (this.$store.getters.getActiveTab !== "generate-new-report") {
                title = this.$store.getters.getModuleName;
                exportArr = this.exportArrAll.filter(
                  (item) => item.path === this.$store.getters.getActiveTab
                );
              } else if (
                this.$store.getters.getActiveTab === "generate-new-report"
              ) {
                exportArr = this.exportArrAll;
                this.exportArrAll = []
              }
            }

            this.mapArr = this.mapValue.filter(
              (item) => item.url == this.$store.getters.getActiveTab
            );
            ...
            let objToAdd = [
              {
                text: {
                  text:
                    this.selectedTemplateTempObj && title
                      ? title + this.selectedTemplateTempObj.templateName
                      : title
                      ? title
            ...
            if (
              this.$store.getters.getActiveTab.includes("generate-new-report")
            ) {
              objToAdd.push({
                text: {
                  text:
                  this.$i18n.t("createdBy") +
                    " :- " +
                    this.selectedTemplateTempObj.createdBy,
                  options: {
                    x: 0.5,
                    y: "92%",
                    w: "60%",
            ...
            objToAdd.push(
              {
                text: {
                  text:
                    ...
                    " :- " +
                    (this.$store.getters.getActiveTab.includes(
                      "generate-new-report"
                    )
                ...
                text: {
                  text:
                    ...
                    (this.$store.getters.getActiveTab.includes(
                      ...
                      ? this.selectedPeriod
                      : `${
                          typeof this.locationPeriod?.periodText === "object"
                            ? this.locationPeriod.periodText.name
                ...
                text: {
                  text:
                    this.$store.getters.getAppSettings.calendar !== "nepali"
                      ? this.$moment(new Date()).format("llll")
                      : this.getNepaliDate(),
                  options: {
                    x: 8.5,
                    y: "98%",
            ...
            for (let index = 0; index < exportArr.length; index++) {
              let element = "";
              if (exportArr[index]?.series && exportArr[index].series.length) {
                element = exportArr[index];
              } else if (
                exportArr[index].chartObj?.series &&
                exportArr[index].chartObj.series.length
              ) {
                element = exportArr[index].chartObj;
              }
              if (element) {
                let slide = pptx.addSlide("MASTER_SLIDE");
                slide.addText(
                  !this.$store.getters.getActiveTab.includes("emuMonthlyTab") &&
                    !this.$store.getters.getActiveTab.includes("emuAnnualTab")
                    ? element?.title?.title
                      ? element.title.title
                      : element.chartName?.[this.$i18n.locale]
                      ? element.chartName[this.$i18n.locale]
                      : element.title
                      ? element.title
                      : ""
                  ...
                  element.series.forEach((e) => {
                    ...
                    if (e.data && e.data.length) {
                      ...
                      e.data.forEach((ed, ind) => {
                        if (ed?.name || ed?.y) {
                          element?.categories
                            ? labels.push(element.categories[ind])
                            : ed?.x
                            ? labels.push(ed.x)
                            : labels.push(ed.name);
                          values.push(ed.y);
        ...
        setTimeout(() => {
          ...
          if (e || this.$refs[divName]) {
            setTimeout(async () => {
              ...
              if(this.$store.getters.getActiveTab.includes("generate-new-report")){
                options.jsPDF = {
                  unit: "px",
                  format: "a4",
                  orientation: "l",
              ...
              let elementToCapture = this.$store.getters.getActiveTab.includes(
                "generate-new-report"
              )
                ? this.$refs[divName]
                : this.currentViewMore != "N/A"
    ...
    addSlideWithImage(pptx, imageData, type, PPTValues) {
      if (type == "bubble") {
        ...
        imageData.map((item) => {
          let title = !item.locVal
            ? item.title
              ? `${item.title}`
              : `${item.location}`
            : PPTValues[1] === "show"
            ? `${item.location} : ${item.locVal}`
            : `${item.location}`;
          const slide = pptx.addSlide("MASTER_SLIDE");
    ...
    updateChartData(chartData, series = undefined) {
      ...
      if (!chartData?.path) {
        chartData["path"] = chartData.viewMore
          ? this.$store.getters.getActiveTab +
            "_" +
            chartData.tabName[this.$i18n.locale]
          : this.$store.getters.getActiveTab;
      }
    ...
    deleteMapPic(dataCurr) {
      this.mapValue = this.mapValue.filter((item) => item.title !== dataCurr);
    },
    deleteBubbleChart() {
      this.bubbleChart = "";
    },
