      ...
      <div class="modal-body">
        ...
        <div class="row">
          <div class="col-12">
            ...
            ><i class="fa fa-2x fa-square-o mr-2 px-2" style="background-color:#abdde5;color:#abdde5" />{{$t('fpSourceStrip1',{factor:2})}}</p>
            <p
              class="textbottom"
              style="font-size:0.75rem;"
            ><i class="fa fa-2x fa-square-o mr-2 px-2" style="background-color:#ffe8a1;color:#ffe8a1" />{{$t('fpSourceStrip1',{factor:10})}}</p>
          </div>
        </div>
      </div>
    </div>
   ...
   /**Note:  need to use return object "combinedObj" in other methods  */
    ...
    drawTable(series, fpItems,combinedObj) {
      ...
      this.methodSeq.forEach(method =>{
          let methodFound = fpItems.filter(obj =>  obj["method"].includes(method));
          if(methodFound.length){
            let mainObj = methodFound[0];
            if(mainObj["method"].includes("Sterilization") || mainObj["method"].includes("Condom")){
      ...
      valObj[Object.keys(valObj)[0]].forEach((v, i) => {
        ...
        if (this.fpItems[i]["submethod"].split('/')[1] == "Male Condom") {
          let variable = fpItems[i]["adjusted"] ? this.$i18n.t('adjusted') :this.$i18n.t('unadjusted') ;
        ...
        Object.keys(valObj).forEach(tab => {
          ...
          if (valObj[tab][i].toFixed(2) * 1 > 2 && valObj[tab][i].toFixed(2) * 1 <= 10) {
