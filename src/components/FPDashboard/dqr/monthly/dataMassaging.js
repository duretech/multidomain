...
const getSlope = (p_obj, _n) => {
    let oFinal = {};
    for (let i in p_obj) {
        let nProd = ((_n * p_obj[i].sumxY) - (p_obj[i].sumX * p_obj[i].sumY)) / ((_n * p_obj[i].sumXsq) - (p_obj[i].sumXofsq));
        oFinal[i] = nProd;
    }
    return oFinal;
}
...
export default {
    ...
    */
    getFormatedData: (p_data, p_global) => {
        let oData = {
            ...
            chartArr: []
        }, i, aChart = p_data.chartData, nLen = aChart.length, oTotalCyp = null, methodSeq = [], mainmethodSeq = [];
        loop1: for (i = 0; i < nLen; i++) {
            ...
            let oOptions = {
                ...
                name: aChart[i].indicator.static_name,
                trans_name: typeof aChart[i].indicator.name == "object" ? aChart[j].indicator.name[0] : aChart[i].indicator.name,
                source: aChart[i].indicator.source
            }, j, aDataElems = [], aSubInd = aChart[i].indicator.subIndicator,
                njLen = aSubInd.length, oCyp = {};
            //console.log(aSubInd)
            for (j = 0; j < njLen; j++) {
                if (aChart[i].indicator.static_name != "Total CYP") {
                    let meName = typeof aSubInd[j].name == "object" ? aSubInd[j].name[0] : aSubInd[j].name
                    methodSeq.push(meName)
                }
                let oDe = {
                    name: typeof aSubInd[j].name == "object" ? aSubInd[j].static_name[0] : aSubInd[j].static_name,
                    trans_name: typeof aSubInd[j].name == "object" ? aSubInd[j].name[0] : aSubInd[j].name,
                    displayName: [], de: [],
                    color: aSubInd[j].color,
                    scaling: aSubInd[j].scalingFactor,
                    subName: Array.isArray(aSubInd[j].static_name) ? aSubInd[j].static_name[0] : aSubInd[j].static_name
                ...
                /*  */
                let mName = typeof aSubInd[j].name == "object" ? aSubInd[j].static_name[0] : aSubInd[j].static_name
            ...
            if (aChart[i].indicator.static_name != 'Total CYP') {
                let mName = typeof aChart[i].indicator.name == "object" ? aChart[i].indicator.name[0] : aChart[i].indicator.name
    ...
    */
    getChartFormatedData: (p_response, p_data, oTarget, mnt) => {
        ...
        if (p_response != undefined && mnt.length) {
            let i, nLen = mnt.length,
                sName = p_data.trans_name + '/' + p_data.subName;
            //sName = p_data.name;
            for (i = 0; i < nLen; i++) {
                let sYear = mnt[i];
                let dataFound = p_response.rows.find(obj => obj[1] == sYear)
                oTarget[sYear][sName] = (oTarget[sYear][sName] || 0) + (dataFound ? dataFound[3] * 1 : 0);
            }
        }
        return oTarget;
    },
    ...
    getTotalCYP: (p_charts) => {
        ...
        for (i = 0; i < nLen; i++) {
            let oCharts = p_charts[i].charts,
                oCyp = p_charts[i].cyp, sName = p_charts[i].name;
            ...
            for (let j in oCharts) {
                for (let k in oCharts[j]) {
                    ...
                    oData[newKey[1]][j] = (oData[newKey[1]][j] || 0) + oCharts[j][k] * oCyp[newKey[1]];
                }
            }
        }
        return oData
    ...
    */
    getFinalTotalCYP: (p_data, p_config, oConfig) => {
        let aCats = [], aSeries = [{ name: oConfig.name, data: [], color: oConfig.color }];
        for (let i in p_data) {
            aCats.push(i);
    ...
    */
    ...
    calculateAdjustedVals: (p_BU, p_CA, p_repoRate, p_cypFactors, p_sumCR, p_cont) => {
    ...
    getFinalOutPutChart: (p_data) => {
        ...
        /*  */
        let nCats = aCats.length, aFinalData = [];
        /*  */
        for (let k in oFinal) {
    ...
    getComparingEstimateModernUsers: (p_data) => {
        let oFinal = {}, aCats = [], aMethodMixCats = [];
        for (let i in p_data) {
            let aSplit = i.split('__'),
                sMethod = aSplit[0],
                sSubM = aSplit[1],
    ...
    getUserTrendsByMethods: (p_cat, p_DHS, p_UNPD, p_data, p_pop, p_bAllWomwn) => {
        let i, nLen = p_cat.length, oFinal = { unpd: [], dhs: [], emuCondms: [], emunoncondoms: [] },
            oCategories = {
                ...
                emuCondms: 'EMU: Commodities inc. Condoms', emunoncondoms: 'EMU: Commodities ex. Condoms'
            },
    ...
    getComparisionofEmuMcpr: (p_data, p_isIncl, p_MICS, p_PMS, p_bAllWomwn) => {
        let oStoreData = {
            [p_bAllWomwn ? 'mCPR (AWRA): MICS' : 'mCPR (MWRA): MICS']: getSlopeOperands(p_MICS),
            [p_bAllWomwn ? 'mCPR (AWRA): PMA' : 'mCPR (MWRA): PMA']: getSlopeOperands(p_PMS)
        ...
        let aCategories = Object.keys(oGlobalSlopes), aFinalData = [],
            nProps = aCategories.length, _nCount = 0;
    ...
    getComparisionofUsersByMethods: (p_data, p_year) => {
        ...
        for (let i in p_data) {
            ...
            let oData = p_data[i].data[p_year] ? p_data[i].data[p_year] : [],
                nLenth = oData.length, j, nInd = oProps[i].index;
    ...
    drawEMUUserbyMethods: (p_data) => {
        ...
        for (let k in p_data) {
            ...
            let aOgData = p_data[k].data,
                nLen = aOgData.length, l, nIndex = oProps[k].index;
            for (l = 0; l < nLen; l++) {
