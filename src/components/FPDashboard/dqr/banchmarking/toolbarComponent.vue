    ...
    <div class="bottom-tabbar-close" @click="closeToolbar"><i class="far fa-times-circle"></i></div>
      <div class="bottom_toolbar_wrap">
        <form class="input-select-items">
          ...
          <div class="form-group pr-3" v-if="recentActiveTab == 'output' || recentActiveTab == 'emu'">
            <label class="text-capitalize">{{$t('year')}}</label>
            <select class="form-control tabbarselect" v-model="emuYear" v-b-tooltip.hover>
              <option v-for="year in emuYears" :key="year.val" :value="year.val">{{year.label}}</option>
            </select>
          </div>
          <div  class="form-group pr-3" v-if="autoSaveSource">
            ...
            <b-button v-if="showAutoButton && autoSaveSource" small @click.prevent.stop="generateEmuButtonClick" class="position-relative mt-3" >
              ...
              <b-button small @click.prevent.stop="viewEMU" class="position-relative mt-3" v-else-if="autoSaveSource">
                <span class="small"> View EMU</span>
              </b-button>
          </div>
        </form>
      </div>
...
export default {
  props: ['defaultYear', 'emuYears', "locationValue", 'showToolbarOnTablet', 'recentActiveTab', 'dqrResponse','autoSaveSource','getActiveTab','generateFlag'],
  components: {
    Treeselect
  },
