    ...
    >
      ...
      <l-control v-if="indicatorMeta" position="bottomright"
        ><span
          ...
          "
          >{{ $t("source") }}:
          {{ indicatorMeta[0].indicator.source }}</span
        ></l-control
      >
    </l-map>
  </div>
...
import { isNumber, formatSingleDate } from "@/components/Common/commonFunctions";
...
export default {
  ...
  methods: {
    ...
    getGeoJson(response) {
      // console.log("getGeoJson response", response)
      if (response) {
    ...
    getIndicatordata(id) {
      // this.showLoader = true;
      // console.log("getIndicatordata", id)
      let sSelectedDE = "";
      sSelectedDE = id;
