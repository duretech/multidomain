...
export default {
  name: "Yearslider",
  props: { yearArr: Array, pType: String, updatedYaerSliderValue: String },
  data() {
    //console.log(this.yearArr, this.pType)
    let curYear = this.updatedYaerSliderValue
  ...
  methods: {
    ...
    play() {
      ...
      var that = this;
      if (that.currentTimePeriodIndex != that.rangeContainer.length) {
        that.periodInterval = setInterval(function () {
          if (index == 0) {
            that.currentTimePeriodIndex = that.isFirstIndex
              ? 1
              : that.currentTimePeriodIndex + 1;
          } else {
            that.currentTimePeriodIndex = that.isFirstIndex
              ? 0
              : that.currentTimePeriodIndex + 1;
          ...
          that.currentYear = that.rangeContainer[that.currentTimePeriodIndex];
          that.playYearInterval(that.currentTimePeriodIndex);
        }, 2000);
      }
    },
    ...
    slideNext() {
      let that = this;
      that.currentIndex = this.rangeContainer.indexOf(that.currentYear);
      if (that.currentIndex == this.maxIndex) {
        this.currentSetter(this.rangeContainer[that.currentIndex]);
      } else {
        this.currentSetter(this.rangeContainer[++that.currentIndex]);
      }
    },
    slidePrevious() {
      let that = this;
      that.currentIndex = this.rangeContainer.indexOf(that.currentYear);
      if (that.currentIndex == this.minIndex) {
        this.currentSetter(this.rangeContainer[that.currentIndex]);
      } else {
        this.currentSetter(this.rangeContainer[--that.currentIndex]);
      }
  ...
  created() {
    ...
    //this.currentYear = this.yearArr[this.yearArr.length-1].split('A')[0];
