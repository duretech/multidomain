...
export default {
  ...
  methods: {
    ...
    getPopulation() {
      ...
      service.getSavedConfig(key3).then((dqrModule) => {
        let id =
    ...
    comparingEMU() {
      ...
      // console.log(noCatSeries, newCategories)
      ...
      if (
        ...
        (this.reportChartData.cid.split("/")[1] === this.chartOptions[0].cid ||
          this.reportChartData.linkedCharts.find((c) =>
            c.includes(this.chartOptions[0].cid)
          ))
      ) {
    ...
    emuTrend() {
      ...
      // console.log("EMU Trend series", series)
      ...
      // console.log("EMU Trend", this.oEMUTrend.tableData)
      // let min = 0, max = 0
      // series.forEach(d => {
      //     // let innerDataArr = d.data.map(dInner => dInner.y)
      //     let innerMin = Math.min(...d.data)
      ...
      let emuData = noCatSeries.find(
        (s) =>
          s.name === this.$i18n.t("total-EMU")
      );
      console.log("emuData", emuData)
      if(emuData) {
        console.log("period", period)
        let currPeriod = this.$moment(period, "MMM YYYY").format("YYYYMM")
        console.log("currPeriod", currPeriod)
        let prevPeriod = this.$moment(period, "MMM YYYY").subtract(1, "months").format("YYYYMM")
        console.log("prevPeriod", prevPeriod)
        
        let cValue = emuData.data.find(d => d.pe === currPeriod)
        console.log("cValue", cValue)
        let pValue = emuData.data.find(d => d.pe === prevPeriod)
        console.log("pValue", pValue)
        if(cValue) {
          this.$emit("currValue", cValue.y)
        } else {
          this.$emit("currValue", 0)
        }
        if(pValue) {
          this.$emit("prevValue", pValue.y)
        } else {
          this.$emit("prevValue", 0)
        }
      } else {
        this.$emit("currValue", 0)
        this.$emit("prevValue", 0)
      ...
      if (
        ...
        (this.reportChartData.cid.split("/")[1] === this.chartOptions[1].cid ||
          this.reportChartData.linkedCharts.find((c) =>
            c.includes(this.chartOptions[1].cid)
          ))
      ) {
    ...
    avgEMUGroth() {
      ...
      if (
        ...
        (this.reportChartData.cid.split("/")[1] === this.chartOptions[2].cid ||
          this.reportChartData.linkedCharts.find((c) =>
            c.includes(this.chartOptions[2].cid)
          ))
      ) {
    ...
    commonMethodUsed(series, categories, source, catIds) {
      ...
      if (series.length > 0) {
        cat.forEach((c, i) => {
          ...
          series.forEach((s) => {
            // console.log(s)
            //console.log(high, s.data[i].value, c, s.data[i].name)
      ...
      if (
        ...
        (this.reportChartData.cid.split("/")[1] === this.chartOptions[3].cid ||
          this.reportChartData.linkedCharts.find((c) =>
            c.includes(this.chartOptions[3].cid)
          ))
      ) {
    ...
    comparingMethods() {
      ...
      catIds.forEach((cat, catIndex) => {
        Object.keys(dataKey).forEach((ids) => {
          if (cat == ids && dataKey[ids] != null) {
            if (this.periodType === "yearly") {
              //console.log(cat, categories[catIndex])
              ...
              if (
                ...
                dataKey[ids]["agreData"].forEach((data) => {
                  ...
                  //  if(ids == "CeLsrJOH0g9"){
                  //     console.log(datVal, sumVal[ids])
                  //     console.log(((datVal/sumVal[ids])*100).toFixed(6)*1)
                  // }
                  methodSeriesObj[newName].push({
                    name: categories[catIndex],
      ...
      if (
        ...
        (this.reportChartData.cid.split("/")[1] === this.chartOptions[4].cid ||
          this.reportChartData.linkedCharts.find((c) =>
            c.includes(this.chartOptions[4].cid)
          ))
      ) {
