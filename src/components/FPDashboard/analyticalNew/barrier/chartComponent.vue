...
export default {
	...
	data() {
		//console.log(this.ministrialData)
	...
	watch: {
		...
		pLength(newValue) {
			if (newValue) {
				this.showLoader = true;
				// console.log("in sendPeriod")
				// console.log("in sendPeriod newValue", newValue)
				// console.log("in sendPeriod oldValue", oldValue)
				this.cObj.series = [];
				this.cObj.drilldown = {
					series: [],
				};
	...
	methods: {
		...
		async getIndicators(minres = this.ministrialData) {
			// console.log("this.btnSelected1",this.btnSelected1)
			// console.log(this.chartRep)
			// console.log(this.ministrialData)
			let moduleConfig = minres;
			// console.log(this.cardKey,moduleConfig)
			// console.log(this.cardKey,this.chartSubCategory)
			// console.log(this.cardKey,this.chartComponent)
			let cData = {};
			...
			// console.log(cData)
			this.cdata = cData;
			// console.log(this.cardKey,cData)
			...
			if (cData) {
				...
				// console.log(this.chartRep,derivedCharts)
				let _this = this;
				...
				if (
					...
					if (!this.derivedChart) {
						...
						// console.log(cData.chartOptions.chart.type.toLowerCase())
				...
				if (this.cyp) {
					// console.log(this.globalFactorsData)
				...
				// console.log(this.chartRep, this.sendLocation)
				//console.log(this.chartSubCategory, catArray)

				let level = [];
				// console.log(this.chartRep, this.facilityLevel)
				...
				// console.log(this.chartRep, level)
				...
				if (
						...
						service
							...
							.then((response) => {
								...
								if (response.data.rows.length) {
									...
									// console.log("fData", this.dataFunction, fData)
									if (fData) {
										...
										// console.log(this.cObj.title.title,this.cObj)
										// console.log(JSON.stringify(this.cObj))
										this.showLoader = false;
										// console.log(this.cObj.series)
					...
					if (this.isRegionalChart) {
						...
						// console.log("this.globalData",this.globalData, this.cardKey, this.sendLocation[1], this.facilityLevel)
						level = [this.sendLocation[1], this.facilityLevel];
						if (this.globalData.de.length > 0 && locID.length > 0) {
							// console.log("this.globalData",this.globalData, this.cardKey)
							let tempSeries = [],
								tempItems = [],
								promises = [];
						...
						if (this.globalData.de.length > 0) {
							service
								.getIndicatorData(sSelectedDE, level, countryID, period)
								.then((response) => {
									// console.log("response",this.btnSelected1,response)
									if (response.data.rows.length) {
										let fData = {
											categories: [],
											series: [],
		 ...
		 */
		dataSort(sort) {
			// console.log("initial data this.cObj", this.cObj)
