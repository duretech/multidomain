				...
				secondatAxisData.forEach((s, i) => {
					...
					axisData.forEach((p, j) => {
						...
						if (
							...
							let drillObj = {
								name: `${name} (${response.data.metaData.items[p].name}) (${response.data.metaData.items[s].name})`,
								id: drillID,
								stack: response.data.metaData.items[s].name,
								type: "column",
								data: [],
				...
				if (chartType !== "pie") {
					...
					axisData.forEach((p, i) => {
						...
						if (Object.keys(rData[d]).includes(p)) {
							...
							items[i] = {
								...items[i],
								[tableHeaders[0].header]: response.data.metaData.items[p].name,
								[name]: rData[d][p] * 1,
							};
						} else {
							obj.data.push({
						...
						if (
							...
							let drillObj = {
								name: `${name} (${response.data.metaData.items[p].name})`,
								id: drillID,
								type: "column",
								data: [],
							};
