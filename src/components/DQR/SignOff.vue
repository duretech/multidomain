					...
					<b-overlay :show="formSubmitted" rounded="sm">
						<div class="signoff-modal-content">
							...
							<div class="dsl" v-if="!showForms && signOffForm.length">
								<div class="title">
									Please choose the data satisfaction level
								</div>
								<div>
...
export default {
	...
	methods: {
			...
			if (index) {
				...
				let signOff = {
					...
					location: this.isUpdate
						...
						: this.locationName,
					period: this.isUpdate ? this.updateForm.period : this.selectedDate,
					signOffTab: this.isUpdate ? this.updateForm.signOffTab : this.tabName,
		...
		refreshForm() {
			...
			this.currentFormID = null;
			this.dataSatisfaction = 0;
			this.signOffForm = JSON.parse(JSON.stringify(this.signOffData.questions));
		},
	},
	created() {
		this.getForms();
