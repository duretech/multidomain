			...
			<template v-for="i in Math.floor(canvasHeight / 1122.24)">
				<div
					...
					v-if="
						...
						(layout.details[0].user || layout.details[1].timestamp)
					"
				>
					<span
						v-if="
					...
					<span
						...
						v-if="
							...
							layout.details &&
							layout.details.length &&
							layout.details[1].timestamp
						"
						>{{ $moment(new Date()).format("lll") }}</span
					>
				</div>
				<div
					class=""
					...
					v-if="
						...
						layout.details &&
						layout.details.length &&
						(layout.details[2].location || layout.details[3].pageCount)
					"
				>
					<span
						v-if="
							...
							layout.details &&
							layout.details.length &&
							layout.details[2].location
						"
					>
						{{ isStaticHeader ? $t("location") : selectedLocationName }}</span
					>
					<span
						class="float-right"
						v-if="
