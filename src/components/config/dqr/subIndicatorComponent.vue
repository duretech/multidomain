<template>
    <div class="row">
        <div class="col-12" v-for="(subInd,_j) in subIndicators" :key="_j">
            <div class="row">
                <div class="col">
                    <a data-toggle="collapse" :href="'#sub_indicator0'+_i+_j"
                        role="button" aria-expanded="false"
                        :aria-controls="'sub_indicator0'+_i+_j"
                        class="pr-2"><i class="fa fa-link  f-s-20px"
                        v-b-tooltip.hover :title="$t('link_IndicatorsData_Elements')">
                        </i>
                    </a>
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="form-group row">
                                        <label class="col-sm-5 col-form-label">{{$t('name')}}</label>
                                        <div class="col-sm-7">
                                            <input type="text" class="form-control" placeholder="" v-model="subInd.name" :disabled="type === 'backgroundIndicators'" />
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-2">
                                            <div class="input-group mb-2">
                                                <input type="color" class="form-control" placeholder="" v-model="subInd.color"/>
                                            </div>
                                        </div>
                                        <div class="col-sm-5">
                                          <input type="text" class="form-control" placeholder="" v-model="subInd.color">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-12">
                                    <div class="card-header bg-faint-grey color-black border-radius-0 text-uppercase f-s-0-875rem">
                                    ...
                                    <div class="card">
                                        <div class="p-t-15px" v-if="bgDataSource !== 'Datastore'">
                                            <div class="form-check form-check-inline pure-checkbox">
                                                <input class="form-check-input" type="radio" :id="'dqrInlineRadio1'+_i+_j"
                                                    value="indicator"
                                                    v-model="subInd.type"
                                                    :checked="subInd.type === 'indicator'"
                                                        @change="reset(_j)"
                                                />
                                                <label class="form-check-label" :for="'dqrInlineRadio1'+_i+_j">{{$t('Indicator')}}</label>
                                            </div>
                                            <div class="form-check form-check-inline pure-checkbox">
                                                <input class="form-check-input" type="radio" :id="'dqrInlineRadio2'+_i+_j"
                                                    value="data_element"
                                                    v-model="subInd.type"
                                                    :checked="subInd.type === 'data_element'"
                                                    @change="reset(_j)"
                                                />
                                                <label class="form-check-label" :for="'dqrInlineRadio2'+_i+_j">{{$t('dataElement')}}</label>
                                            </div>
                                            <div class="form-check form-check-inline pure-checkbox">
                                                <input class="form-check-input" type="radio" :id="'dqrInlineRadio3'+_i+_j"
                                                    value="data_sets"
                                                    v-model="subInd.type"
                                                    :checked="subInd.type === 'data_sets'"
                                                    @change="reset(_j)"
                                                />
                                        ...
                                        <div class="card-body">
                                            <div class="row" v-if="bgDataSource !== 'Datastore'">
                                                <div class="col-12" v-show="subInd.type === 'indicator'">
                                                    <div class="row">
                                                        <div class="col-12">
                                                            <b-button class="black-btn mx-1" v-b-modal="'dqrModal_indicator'+_i+_j">{{$t('addbtn')}} {{$t('Indicator')}}</b-button>
                                                            ...
                                                            <b-modal :id="'dqrModal_indicator'+_i+_j" hide-footer :title="$t('selectIndicator')" no-close-on-backdrop>
                                                                <div>
                                                                    {{$t('selectIndicator')}}
                                                                    <multiselect v-model="selectedIndicator" select-label=""
                                                                        track-by="id" label="displayName"
                                                                        :placeholder="$t('search_data_element')" :hide-selected="true"
                                                                        :options="indicatorsList" :searchable="true" :allow-empty="false">
                                                ...
                                                <div class="col-12" v-show="subInd.type === 'data_element'">
                                                    <div class="row">
                                                        <div class="col-12">
                                                            ...
                                                            <b-modal :id="'dqrModal_data_element'+_i+_j" hide-footer
                                                            :title="$t('selectDataElement')" no-close-on-backdrop>
                                                                <div>
                                                                    {{$t('selectDataElement')}}
                                                                    <multiselect v-model="selectedDataelement" select-label="" track-by="id"
                                                                        label="displayName" :placeholder="$t('selectDataElement')"
                                                                        :options="dataElementsList" :searchable="true" :allow-empty="false">
                                                                        <template slot="singleLabel" slot-scope="{ option }">{{ option.displayName }}</template>
                                                                    </multiselect>
                                                                </div>
                                                                <div v-if="selectedDataelement" class="pt-3">
                                                                    {{$t('selectCategoryCombo')}}
                                                                    <multiselect v-model="selectedCategoryCombo" select-label="" track-by="id" label="displayName" :placeholder="$t('search_category_combo')" :options="selectedDataelement.categoryCombo.categoryOptionCombos" :searchable="true" :allow-empty="false">
                                                                    <template slot="singleLabel" slot-scope="{ option }">{{ option.displayName }}</template>
                                                                    </multiselect>
                                                                </div>
                                                                <div class="text-right pt-3">
                                            ...
                                            <div class="row" v-else>
                                              <div class="col-12 pt-3" v-if="subInd.selectedDatastoreDE.length > 0">
                                                  {{$t('selectedDataElements')}}
                                                  <ul  class="list-unstyled">
        ...
        methods:{
            ...
            add(_j,type){
                ...
                if(type == "indicator"){
                    ...
                    //console.log(this.selectedDataelement);
                    //console.log(this.selectedCategoryCombo);
                    let selectedDE = {};
            ...
            dataEntry(id) {
              // console.log("clicked subIndicators", id)
              this.dataEntryID = id
