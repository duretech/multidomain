...
export default {
  ...
  getFormatedData: (p_data, p_global, startingYear, cyp, continuation) => {
    ...
    let oData = {
      ...
      emuColors = {},
      methodSeq = [],
      monthlyMethodSeq = [], //developed for IC
      tableMethodSeq = [],
      mainmethodSeq = [];
      ...
      for (j = 0; j < njLen; j++) {
        ...
        let statName =
          typeof aChart[i].indicator.static_name == "object"
            ? aChart[i].indicator.static_name[i18n.locale]
            : aChart[i].indicator.static_name;
        monthlyMethodSeq.push(subIndNameAct);
    ...
    return {
      ...
      monthlyMethodSeq: monthlyMethodSeq,
    };
  },
  /**
   * This function is to get formated data as per charts
   ...
   *
   */
  calculateSTMNotAdjusted: (p_arr, p_repo, p_adjF, disablerepo) => {
  ...
  getFinaladjNonAdjData: (allMethodsAdjusted) => {
  ...
  getYearFormatedMonthly: (p_start, p_end) => {
    let sCurYear = p_end * 1,
      nStartYear = p_start * 1,
      aYear = [nStartYear];
    while (nStartYear < sCurYear) {
  ...
  getChartFormatedDataMonthly: (p_response, p_data, oTarget, mntStr) => {
  ...
  getemuComparisonData: (
    ...
    let oProps = {
      Commodities_Client: i18n.t("emu_output_5"),
      Commodities_Facilities: i18n.t("emu_output_6"),
      Users: i18n.t("emu_output_8"),
      Visits: i18n.t("emu_output_7"),
    };
    let actTypes = {
    ...
    for (let type in oProps) {
      ...
      let emuFilter = filter && filter[type] ? filter[type] : "inc"; //static inc for single page emu o/pchrats
  ...
  getuserTrendsbyEmu: (sYear, methodWiseAdjObject, oPopulation) => {
  ...
  emuOutputCompByMethod: (
    ...
    console.log(oProps, defaultEMU, finalMethodArr, "in datamassaging method");
