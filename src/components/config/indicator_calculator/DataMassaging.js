 ...
 */
const getYearFormated = (p_start, p_end) => {
...
const getFormatedBackGroundData = (p_data, p_Keys, p_year, popYear) => {
  /***Added by ashvini */
  ...
  Object.keys(p_Keys).forEach((methodName) => {
    if (p_Keys[methodName].length) {
      ...
      if (p_Keys[methodName].length > 1) {
        ...
        finalBG[methodName] = {};
        let dataRows = p_data.rows;
        for (let index = 0; index < popYear.length; index++) {
          let eachYear = popYear[index];
          let iFound = false;
          iFound = dataRows.find((rowArray) => {
            //console.log(rowArray,methodDE,eachYear);
            if (rowArray[0] === methodDE[0] && rowArray[1] === eachYear) {
              finalBG[methodName][eachYear] = (rowArray[3] * 1).toFixed(3) * 1;
              return true;
            }
          });
...
const getFormatedData = (
  ...
  loop1: for (i = 0; i < nLen; i++) {
    ...
    let oOptions = {
        ...
        name: aChart[i].indicator.name + "/" + aChart[i].indicator.static_name,
    ...
    for (j = 0; j < njLen; j++) {
      ...
      if (aChart[i].indicator.static_name === "Sterilization") {
        ...
        finalMethodArr.push(
          Array.isArray(aSubInd[j].name) ? aSubInd[j].name[0] : aSubInd[j].name
        );
      }
      ...
      let oDe = {
        name: aSubInd[j].name + "/" + subIndName,
      ...
      // /
      oCyp[aSubInd[j].name + "/" + subIndName] = cyp[subIndName];
      oDe.de = aSubInd[j].de;
...
/***
 * TO get formated continuation factor
 * p_cont is globalFactor continuation data
 */
const getComputedContFact = (p_cont) => {
  for (let i in p_cont) {
...
const getFpSourceVals = (p_modified, p_fpItems) => {
 ...
 */
const getChartFormatedData = (p_response, p_data, oTarget, yearString) => {
  //   console.log(
  //     p_response,
  //     p_data,
  //     JSON.parse(JSON.stringify(oTarget)),
  //     yearString
  //   );
...
// //      *
// //     */
const calculateSTMNotAdjusted = (p_arr, p_repo, p_adjF, disablerepo) => {
  ...
  for (i = 0; i < nLen; i++) {
    ...
    if (bIsShortTerm === "Short Term") {
      oNotAdjusted[sName] = {};
      oAdjusted[sName] = {};
      //console.log("in chart loop",oChart)
      for (let j in oChart) {
        // console.log("in chart loop")
        oNotAdjusted[sName][j] = {};
        oAdjusted[sName][j] = {};
        for (let k in oChart[j]) {
          // console.log("in oChart[j] loop")

          oNotAdjusted[sName][j][k] = oChart[j][k];
          oAdjusted[sName][j][k] = oChart[j][k];
          if ((p_repo != null && p_repo[j] > 60) || disablerepo) {
            // console.log("in p_repo")
  ...
  //   console.log(
  //     JSON.parse(JSON.stringify(oAdjusted)),
  //     JSON.parse(JSON.stringify(oNotAdjusted))
  //   );
  return {
    adjusted: oAdjusted,
...
const calculateNewBU = (
    ...
    for (let j in oBU[method]) {
      ...
      for (let sSubM in oBU[method][j]) {
        if (sSubM !== "scaling") {
          ...
          if (p_repo && p_repo[j] < 60 && !disablerepo) {
  ...
  console.log("continuation factor", p_cont);
  return {
    scalling: scalling,
    baseline: oBU,
    adjusted: oAdjusted,
...
const calculateNewAdjustedVals = (p_AdjtVal) => {
  let finalAdjusted = {};
...
// //     */
const getFinalChartData = (p_data, p_config) => {
 ...
 * @return {Object} formated data to be used in HighCharts
 */
const getFinalTotalCYP = (p_data, p_config, oConfig) => {
  ...
  let aCats = [],
    ...
    aTableData = [],
    aFileds = [{ key: "Period", sortable: true }, "Total CYP"],
...
const getFinaladjNonAdjData = (allMethodsAdjusted) => {
...
const getYearFormatedMonthly = (p_start, p_end) => {
  let sCurYear = p_end * 1,
    nStartYear = p_start * 1,
    aYear = [nStartYear];
  while (nStartYear < sCurYear) {
...
const getSumOfContMonthly = (p_obj) => {
  let oFinal = {};
  for (let i in p_obj) {
    oFinal[i] = [];
    let nSum = 0,
...
const getuserTrendsbyEmuForSlope = (
  sYearArray,
  methodWiseAdjObject,
  oPopulation,
  finalObj,
...
const getuserTrendsbyEmu = (sYear, methodWiseAdjObject, oPopulation) => {
  //console.log(sYear, methodWiseAdjObject, oPopulation);
  ...
  for (let yearInd in sYearArray) {
    ...
    let exCondomVal =
      ((totalObj[year] && totalObj[year]["exCondom"]
        ...
        (oPopulation[year] ? oPopulation[year] : 0)) *
      100;
    finalObj["incCondom"]["data"][year] = incCondomVal.toFixed(2) * 1;
    finalObj["exCondom"]["data"][year] = exCondomVal.toFixed(2) * 1;
  }
  return finalObj;
};
const getChartFormatedDataMonthly = (p_response, p_data, oTarget, mntStr) => {
  let mnt = mntStr.split(";");
  if (p_response !== undefined && mnt.length) {
    let i,
      nLen = mnt.length,
      sName = p_data.trans_name + "/" + p_data.static_name;
    //sName = p_data.name;
    for (i = 0; i < nLen; i++) {
      let sYear = mnt[i];
      if (!oTarget[sYear]) {
        oTarget[sYear] = {};
        oTarget[sYear][sName] = 0;
      }
      let dataFound = p_response.rows.find((obj) => {
        if (obj[1] * 1 === sYear * 1) return obj;
        else return null;
      });
      oTarget[sYear][sName] =
  ...
  for (let yrInd in sYearArray) {
    ...
    for (let type in allData) {
      ...
      for (let method in allData[type]) {
        let val = allData[type][method][year] ? allData[type][method][year] : 0;
        sum = sum + val * 1;
      }
    ...
    if (year == endYear) {
      for (let methodInd in finalMethodArr) {
        ...
        let nonAdjustmentVal =
          allData["nonAdjusted"][method] && allData["nonAdjusted"][method][year]
...
const combinedComparisonEstimate = (
  endYear,
  finalMethodArr,
  sYearArray,
  methodWiseObject,
...
const comarisonEstimateColumnChart = (
  endYear,
  finalMethodArr,
  sYearArray,
  methodWiseObject,
...
const getUserTrendsDatafromSurvey = (
  sYearArray,
  unpdData,
  dhsData,
  pmaData,
...
const getemuComparisonData = (bgSureyData, outputData, filter) => {
  let oProps = {
...
const getSlopData = (userTrendsbyEmu) => {
