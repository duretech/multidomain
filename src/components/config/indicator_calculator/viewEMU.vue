...
export default {
  ...
  data() {
    return {
      ...
      optionsDataType: [
        ...
        { item: "Commodities_Client", name: this.$i18n.t('Commodities_Client') },
        { item: "Commodities_Facilities", name: this.$i18n.t('Commodities_Facilities') },
    ...
    rows() {
      ...
      return this.items.filter((item) => {
        return (
          ...
          item[this.$i18n.t("period")].toString().includes(query) ||
  ...
  methods: {
    async printTable() {
      ...
      let isExistDEAr, sYear;
      if (this.location && this.selectedEMUType && this.selectedDataType) {
        let periodData;
        if (Object.keys(this.$store.getters.getGlobalFactors('fp_dashboard')).length == 0) {
          let key = this.generateKey("globalFactors")
          await service.getSavedConfig({ tableKey:key, namespace:'fp-dashboard' }).then((resp)=>{
            periodData = resp.data.period.Period;
          })
        }
        else{
          periodData = this.$store.getters.getGlobalFactors(
            "fp-dashboard",
            false
          ).period.Period;
        }
        ...
        if (isExistDEAr.length) {
          ...
          console.log("actDEID", actDEID);
          console.log("loc", locs);
          let locName = [];
          if (locs) {
            locs.split(";").map(async(item) => {
              await service.getIndividualOrganisation(item).then((resp) => {
                locName.push({ [`${item}`]: resp.data.name });
              });
            });
